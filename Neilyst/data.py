...
def get_klines(
    ...
    if isinstance(symbol, str):
        # 处理单个 symbol 的情况
        return _get_single_symbol_klines(
            symbol,
            start,
            end,
            timeframe,
        ...
        all_data = {}
        for sym in symbol:
            data = _get_single_symbol_klines(
                sym,
                start,
                end,
                timeframe,
...
def aggregate_custom_timeframe(
    symbol,
    start,
    end,
    custom_timeframe,
    ...
    if data_path is None:
        ...
        data_path_1m = os.path.join(
            data_path, f"{exchange_name}-{symbol_sp[0]}", timeframe
        )
    ...
    if auth:
        ...
        if missing_periods:
            ...
            for period in format_missing_periods:
                ...
                klines_1m = _fetch_klines(
                    symbol, start_time, end_time, timeframe, exchange
                )
    ...
    # 将原始index设置为对应的时间点
    aggregated_klines.index = all_klines.index[::custom_minutes][
        : len(aggregated_klines)
    ]

    # 保存聚合后的数据
    if data_path is None:
        custom_data_path = (
            f"{current_path}/data/{exchange_name}-{symbol_sp[0]}/{custom_timeframe}"
        )
    else:
        custom_data_path = os.path.join(
            data_path, f"{exchange_name}-{symbol_sp[0]}", custom_timeframe
        )
    ...
    while current_date <= end_date_adjusted:
        ...
        while time_pointer < day_end:
            if not any(start <= time_pointer < end for start, end in covered):
                ...
                while time_pointer < day_end and not any(
                    start <= time_pointer < end for start, end in covered
                ):
                    time_pointer += timedelta(minutes=1)
                missing_end = time_pointer
                missing_data.append(
                    f"{missing_start.strftime('%Y-%m-%d-%H:%M')} - {missing_end.strftime('%Y-%m-%d-%H:%M')}"
                )
            else:
                time_pointer += timedelta(minutes=1)
